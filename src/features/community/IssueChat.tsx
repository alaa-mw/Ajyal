...
const IssueChat = ({ issue }: Props) => {
  ...
  return (
    <>
      <Typography variant="h6" mt={3} fontWeight={"bold"}>المناقشة:</Typography>
          ...
          >
            {isLoading ? (
              ...
              <Stack spacing={2}>
                {replies?.map(
                  (
                    r: Reply // make if from teacher algin left else student
                  ) => (
                    <Box
                      key={r.id}
                      sx={{
                        alignSelf: getAlignSelf(r.author_type),
                        width: "80%",
                        p: 1.5,
                        borderRadius: 2,
                        bgcolor: getBgColor(r.author_type),
                        boxShadow: 1,
                      }}
                    >
